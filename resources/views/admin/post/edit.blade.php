                ...
                <div class="bg-white border-b mt-3 dark:bg-gray-900 dark:border-gray-700 hover:bg-gray-50 rounded-[10px]">
                    ...
                    <div class="m-8">
                        <label class="block mb-4 text-gray-500 font-bold mt-4 ml-2">
                            <i class="fas fa-info-circle mr-2"></i>Status: <span class="text-black">{{ $post->status ? 'Published' : 'Draft' }}
                            <input type="checkbox" name="status" {{ $post->status ? 'checked' : '' }} value="1"></span>
                        </label>
                        
                        <label class="block mb-4 text-gray-500 font-bold mt-4 ml-2">
                            <i class="fas fa-eye mr-2"></i>Visibility: <span class="text-black">{{ $post->visibility ? 'Public' : 'Hidden' }}
                            <input type="checkbox" name="visibility" {{ $post->visibility ? 'checked' : '' }} value="1"></span>
                        </label>
                    
                        <label class="block mb-4 text-gray-500 font-bold mt-4 ml-2">
                            <i class="fas fa-history mr-2"></i>Revisions: <span class="text-black">{{ $post->revisions }}</span>
                        </label>
                    
                        <label class="block mb-4 text-gray-500 font-bold mt-4 ml-2">
                            <i class="far fa-clock mr-2"></i>Last Updated: <span class="text-black">{{ $post->updated_at }}</span>
                        </label>
                    </div>
